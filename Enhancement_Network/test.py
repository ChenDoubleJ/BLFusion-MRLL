...
def main():
    ...
    train_visible_path, train_infrared_path, val_visible_path, val_infrared_path = read_data("dataset/MRLL")
    ...
    # model_weight_path = "./experiments/TDN_train_20251204-102557/weights/checkpoint_realscene.pth"
    ...
    for img in val_loader:
        ...
        if torch.cuda.is_available():
            ...
            ir = ir.to(device)
        fir = ir[:, :1, :, :]
        # input = torch.cat((vis, fir), dim = 1)
        input = vis
        # print(input.size())
        with torch.no_grad():

            vi_r, vi_l = model(input)
        ...
        savepic(vi_l, name[0], flag="vi_l")
        # savepic(ir_l, name[0], flag="ir_l")
