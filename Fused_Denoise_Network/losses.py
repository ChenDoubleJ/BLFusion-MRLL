...
class fusion_loss(nn.Module):
    def __init__(self):
        ...
        self.loss_func_ssim = L_SSIM(window_size=11)  # 48
        self.loss_func_Grad = L_Grad_position()
        self.loss_func_Max = L_Intensity()
        self.loss_func_color = L_color()
    
    def forward(self, image_visible, image_infrared, image_fused, max_ratio=0.2, ssim_vis_ratio=1, ssim_ir_ratio=1, ssim_ratio=0.05, color_ratio=1.2, grad_ratio=0.2, l1_ratio = 0.08):
        ...
        loss_l1 = l1_ratio * (F.l1_loss(image_fused, image_visible) + F.l1_loss(image_fused, image_infrared))

        total_loss = loss_max + loss_ssim + loss_color + loss_grad + loss_l1
        return total_loss, loss_ssim, loss_max, loss_color, loss_grad, loss_l1
