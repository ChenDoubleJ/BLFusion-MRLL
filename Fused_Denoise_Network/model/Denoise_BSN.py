...
class Denoise_BSN(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=3, base_ch=128, num_module=9, mask_type='o_fsz'):
        ...
        if 'o' in self.mask_types:  # 2 3
            ...
            self.branch1_3 = DC_branchl(5, base_ch, 'central', num_module)
    ...
    def forward(self, x):
        ...
        x = self.head(x)
        # print(x.size())
        # torch.save(x.cpu(), 'before.pt')
        if 'o' in mask_types:
            br1_1 = self.branch1_1(x)
            br1_2 = self.branch1_2(x)
            # br1_3 = self.branch1_3(x)
            y.append(br1_1)
            y.append(br1_2)
            # y.append(br1_3)
        if 'c' in mask_types:
...
class DC_branchl(nn.Module):
    def __init__(self, stride, in_ch, mask_type, num_module):
        ...
        if mask_type == 'r':
            ...
            # ly += [nn.Conv2d(in_ch, in_ch, kernel_size=2 * stride - 1, stride=1, padding=stride - 1)]
